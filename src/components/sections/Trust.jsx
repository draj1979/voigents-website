...
const Trust = () => {
    const items = [
        {
            icon: <Shield size={32} color="var(--accent-cyan)" />,
        ...
        {
            icon: <Eye size={32} color="var(--accent-cyan)" />,
        ...
        <section id="trust" className="section-padding">
            <div className="container">
                <div className="text-center" style={{ marginBottom: '64px' }}>
                    <h2 style={{ marginBottom: '16px' }}>Built for Trust & Control</h2>
                    <p className="text-secondary">Enterprise reliability with the flexibility of modern AI.</p>
                </div>

                <div className="grid grid-3">
                    {items.map((item, index) => (
                        <div key={index} className="flex flex-col items-center text-center">
                            <div className="flex-center glass" style={{ width: '80px', height: '80px', borderRadius: '50%', marginBottom: '24px' }}>
                                {item.icon}
                            </div>
                            <h3 style={{ marginBottom: '16px' }}>{item.title}</h3>
                            <p className="text-secondary" style={{ maxWidth: '300px' }}>{item.description}</p>
                        </div>
                    ))}
                </div>
            </div>
